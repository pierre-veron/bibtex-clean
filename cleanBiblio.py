...
def main():
    ...
    cleanBiblio(input, output, abbreviate, dotless)

def cleanBiblio(input, output, abbreviate, dotless):
    """Clean and check a bibtex file for a LaTeX document.

    Args:
        input (str): path to the input .bib file
        output (str): path to the output .bib file (if already exists, is replaced)
        abbreviate (bool): abbreviates the journal name
        dotless (bool): if abbreviate, without dots (e.g. Nat Ecol Evol instead 
            of Nat. Ecol. Evol.)

    Raises:
        KeyError: if some important keys are missing in some entries (articles
            must have a journal or journal title)
    """
    ...
    print("\n" + "-"*30)
    print("Bibliography cleaned and stored to {}".format(output))
